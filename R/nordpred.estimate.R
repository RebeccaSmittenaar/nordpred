...
#'  
#' 'nordpred.estimate' estimates parameters in the power5 or poisson 
...
#' where the link is g(x)=x^0.2, while the alternative is the poisson function 
#' ("poisson"), where the link is g(x)=log(x)
#' 
#' @return An object of class \code{nordpred.estimate}.
#' 
#' @references 
#' \itemize{
...
nordpred.estimate <- function(cases,pyr,noperiod,startestage,linkfunc="power5", find_best_model = FALSE) {
